...
export class TG3Component implements OnInit {
  ...
  defconsoChart: any;
  rendementChart: any;
  constructor(private birmchergatg3Service: Birmchergatg3Service, private http: HttpClient) { }
  ...
  getbirmchergastg3(): void {
    this.birmchergatg3Service.getVal().subscribe(birmchergastg3 => {
      ...
      this.birmchergastg3 = birmchergastg3.map(data => ({
        ...data,
        Date_Birmchergatg3: data.Date_Birmchergatg3 ? new Date(data.Date_Birmchergatg3) : undefined
      }));
      console.log("Récuperation des données:", this.birmchergastg3);

      // Preparing data for the charts
      const labels = this.birmchergastg3
        .filter(data => !!data.Date_Birmchergatg3) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => this.formatDate(data.Date_Birmchergatg3!)); // Use ! to indicate that Date_Birmchergatg4 cannot be undefined
      const degradationData = this.birmchergastg3
        .filter(data => !!data.Date_Birmchergatg3) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.Tauxdegradation_Birmchergatg3);
      const gasConsumptionData = this.birmchergastg3
        .filter(data => !!data.Date_Birmchergatg3) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.consogaz_Birmchergatg3);
      const defconsoData = this.birmchergastg3
        .filter(data => !!data.Date_Birmchergatg3) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.defconso_Birmchergatg3);
      const rendementData = this.birmchergastg3
        .filter(data => !!data.Date_Birmchergatg3) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.rendement_Birmchergatg3);
  ...
  downloadExcel(): void {
    this.http.get('http://localhost:8000/export/excel', { responseType: 'arraybuffer' })
      .subscribe((data: any) => {
        ...
        a.href = url;
        a.download = 'birmchergatg3_export.xls';
