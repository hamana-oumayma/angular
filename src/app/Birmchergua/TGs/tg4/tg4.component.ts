...
export class TG4Component implements OnInit {
  ...
  defconsoChart: any;
  rendementChart: any;
  constructor(private birmchergatg4Service: Birmchergatg4Service, private http: HttpClient) { }
  ...
  getbirmchergastg4(): void {
    this.birmchergatg4Service.getVal().subscribe(birmchergastg4 => {
      ...
      this.birmchergastg4 = birmchergastg4.map(data => ({
        ...data,
        Date_Birmchergatg4: data.Date_Birmchergatg4 ? new Date(data.Date_Birmchergatg4) : undefined
      }));
      console.log("Récuperation des données:", this.birmchergastg4);

      // Preparing data for the charts
      const labels = this.birmchergastg4
        .filter(data => !!data.Date_Birmchergatg4) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => this.formatDate(data.Date_Birmchergatg4!)); // Use ! to indicate that Date_Birmchergatg4 cannot be undefined
      const degradationData = this.birmchergastg4
        .filter(data => !!data.Date_Birmchergatg4) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.Tauxdegradation_Birmchergatg4);
      const gasConsumptionData = this.birmchergastg4
        .filter(data => !!data.Date_Birmchergatg4) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.consogaz_Birmchergatg4);
      const defconsoData = this.birmchergastg4
        .filter(data => !!data.Date_Birmchergatg4) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.defconso_Birmchergatg4);
      const rendementData = this.birmchergastg4
        .filter(data => !!data.Date_Birmchergatg4) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.rendement_Birmchergatg4);
  ...
  downloadExcel(): void {
    this.http.get('http://localhost:8000/export/excel', { responseType: 'arraybuffer' })
      .subscribe((data: any) => {
        ...
        a.href = url;
        a.download = 'birmchergatg4_export.xls';
