...
export class TG1Component implements OnInit {
  ...
  getbirmchergatg1(): void {
    this.birmchergatg1Service.getVal().subscribe(birmchergastg1 => {
      ...
      console.log("Récuperation des données:", this.birmchergastg1);
  
      // Preparing data for the charts
      const labels = this.birmchergastg1
      .filter(data => !!data.Date_Birmchergatg1) // Filter objects where Date_Birmchergatg4 is defined
      .map(data => this.formatDate(data.Date_Birmchergatg1!)); // Use ! to indicate that Date_Birmchergatg4 cannot be undefined
    const degradationData = this.birmchergastg1
      .filter(data => !!data.Date_Birmchergatg1) // Filter objects where Date_Birmchergatg4 is defined
      .map(data => data.Tauxdegradation_Birmchergatg1);
      

      const gasConsumptionData = this.birmchergastg1
        .filter(data => !!data.Date_Birmchergatg1) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.consogaz_Birmchergatg1);
      const defconsoData = this.birmchergastg1
        .filter(data => !!data.Date_Birmchergatg1) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.defconso_Birmchergatg1);
       const rendementData = this.birmchergastg1
        .filter(data => !!data.Date_Birmchergatg1) // Filter objects where Date_Birmchergatg4 is defined
        .map(data => data.rendement_Birmchergatg1);
