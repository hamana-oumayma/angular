...
defconsoChart: any;
rendementChart: any;
constructor(private bouchematg4Service: Bouchematg4Service, private http: HttpClient) { }
...
getbouchematg4(): void {
  this.bouchematg4Service.getVal().subscribe(bouchematg4 => {
    ...
    this.bouchematg4 = bouchematg4.map(data => ({
      ...data,
      Date_Bouchematg4: data.Date_bouchematg4 ? new Date(data.Date_bouchematg4) : undefined
    }));
    console.log("Récuperation des données:", this.bouchematg4);

    // Preparing data for the charts
    const labels = this.bouchematg4
      .filter(data => !!data.Date_bouchematg4) // Filter objects where Date_Bouchematg4 is defined
      .map(data => this.formatDate(data.Date_bouchematg4!)); // Use ! to indicate that Date_Bouchematg4 cannot be undefined
    const degradationData = this.bouchematg4
      .filter(data => !!data.Date_bouchematg4) // Filter objects where Date_Bouchematg4 is defined
      .map(data => data.Tauxdegradation_bouchematg4);
    const gasConsumptionData = this.bouchematg4
      .filter(data => !!data.Date_bouchematg4) // Filter objects where Date_Bouchematg4 is defined
      .map(data => data.consogaz_bouchematg4);
    const defconsoData = this.bouchematg4
      .filter(data => !!data.Date_bouchematg4) // Filter objects where Date_Bouchematg4 is defined
      .map(data => data.defconso_bouchematg4);
    const rendementData = this.bouchematg4
      .filter(data => !!data.Date_bouchematg4) // Filter objects where Date_Bouchematg4 is defined
      .map(data => data.rendement_bouchematg4);
...
this.http.get('http://localhost:8000/export/excel', { responseType: 'arraybuffer' })
  .subscribe((data: any) => {
    ...
    a.href = url;
    a.download = 'bouchematg4_export.xls';
