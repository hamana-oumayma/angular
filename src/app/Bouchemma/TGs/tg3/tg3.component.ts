...
defconsoChart: any;
rendementChart: any;
constructor(private bouchematg3Service: Bouchematg3Service, private http: HttpClient) { }
...
getbouchematg3(): void {
  this.bouchematg3Service.getVal().subscribe(bouchematg3 => {
    // Assigning data without filtering
    this.bouchematg3 = bouchematg3.map(data => ({
      ...data,
      Date_Bouchematg3: data.Date_bouchematg3 ? new Date(data.Date_bouchematg3) : undefined
    }));
    console.log("Récuperation des données:", this.bouchematg3);

    // Preparing data for the charts
    const labels = this.bouchematg3
      .filter(data => !!data.Date_bouchematg3) // Filter objects where Date_Bouchematg3 is defined
      .map(data => this.formatDate(data.Date_bouchematg3!)); // Use ! to indicate that Date_Bouchematg3 cannot be undefined
    const degradationData = this.bouchematg3
      .filter(data => !!data.Date_bouchematg3) // Filter objects where Date_Bouchematg3 is defined
      .map(data => data.Tauxdegradation_bouchematg3);
    const gasConsumptionData = this.bouchematg3
      .filter(data => !!data.Date_bouchematg3) // Filter objects where Date_Bouchematg3 is defined
      .map(data => data.consogaz_bouchematg3);
    const defconsoData = this.bouchematg3
      .filter(data => !!data.Date_bouchematg3) // Filter objects where Date_Bouchematg3 is defined
      .map(data => data.defconso_bouchematg3);
    const rendementData = this.bouchematg3
      .filter(data => !!data.Date_bouchematg3) // Filter objects where Date_Bouchematg3 is defined
      .map(data => data.rendement_bouchematg3);
...
.subscribe((data: any) => {
  ...
  a.href = url;
  a.download = 'bouchematg3_export.xls';
