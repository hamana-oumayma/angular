...
export class TG1Component implements OnInit {
  ...
  constructor(private radestg1Service: Radestg1Service, private http: HttpClient) { }
  ...
  getRadestg1(): void {
    this.radestg1Service.getVal().subscribe(radestg1 => {
      ...
      this.radestg1 = radestg1.map(data => ({
        ...data,
        Date_Radestg1: data.Date_radestg1 ? new Date(data.Date_radestg1) : undefined
      }));
      console.log("Récuperation des données:", this.radestg1);
  
      // Preparing data for the charts
      const labels = this.radestg1
        .filter(data => !!data.Date_radestg1) // Filter objects where Date_Radestg1 is defined
        .map(data => this.formatDate(data.Date_radestg1!)); // Use ! to indicate that Date_Radestg1 cannot be undefined
      const degradationData = this.radestg1
        .filter(data => !!data.Date_radestg1) // Filter objects where Date_Radestg1 is defined
        .map(data => data.Tauxdegradation_radestg1);
      const gasConsumptionData = this.radestg1
        .filter(data => !!data.Date_radestg1) // Filter objects where Date_Radestg1 is defined
        .map(data => data.consogaz_radestg1);
      const defconsoData = this.radestg1
        .filter(data => !!data.Date_radestg1) // Filter objects where Date_Radestg1 is defined
        .map(data => data.defconso_radestg1);
      const rendementData = this.radestg1
        .filter(data => !!data.Date_radestg1) // Filter objects where Date_Radestg1 is defined
        .map(data => data.rendement_radestg1);
...
.subscribe((data: any) => {
  ...
  a.href = url;
  a.download = 'radestg1_export.xls';
