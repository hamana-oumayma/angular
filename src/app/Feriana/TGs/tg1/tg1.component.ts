...
export class TG1Component implements OnInit {
  ...
  constructor(private ferianatg1Service: Ferianatg1Service, private http: HttpClient) { }
  ...
  getFerianatg1(): void {
    this.ferianatg1Service.getVal().subscribe(ferianatg1 => {
      ...
      this.ferianatg1 = ferianatg1.map(data => ({
        ...data,
        Date_Ferianatg1: data.Date_ferianatg1 ? new Date(data.Date_ferianatg1) : undefined
      }));
      console.log("Récuperation des données:", this.ferianatg1);
  
      // Preparing data for the charts
      const labels = this.ferianatg1
        .filter(data => !!data.Date_ferianatg1) // Filter objects where Date_Ferianatg1 is defined
        .map(data => this.formatDate(data.Date_ferianatg1!)); // Use ! to indicate that Date_Ferianatg1 cannot be undefined
      const degradationData = this.ferianatg1
        .filter(data => !!data.Date_ferianatg1) // Filter objects where Date_Ferianatg1 is defined
        .map(data => data.Tauxdegradation_ferianatg1);
      const gasConsumptionData = this.ferianatg1
        .filter(data => !!data.Date_ferianatg1) // Filter objects where Date_Ferianatg1 is defined
        .map(data => data.consogaz_ferianatg1);
      const defconsoData = this.ferianatg1
        .filter(data => !!data.Date_ferianatg1) // Filter objects where Date_Ferianatg1 is defined
        .map(data => data.defconso_ferianatg1);
      const rendementData = this.ferianatg1
        .filter(data => !!data.Date_ferianatg1) // Filter objects where Date_Ferianatg1 is defined
        .map(data => data.rendement_ferianatg1);
...
downloadCsv(): void {
this.http.get('http://localhost:8000/export/csv', { responseType: 'text' })
  .subscribe((data: any) => {
    this.downloadFile(data, 'birmchergatg1_export.csv', 'text/csv');
  });
}
