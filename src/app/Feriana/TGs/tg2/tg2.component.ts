...
constructor(private ferianatg2Service: Ferianatg2Service, private http: HttpClient) { }
...
getFerianatg2(): void {
  this.ferianatg2Service.getVal().subscribe(ferianatg2 => {
    ...
    this.ferianatg2 = ferianatg2.map(data => ({
      ...data,
      Date_Ferianatg2: data.Date_ferianatg2 ? new Date(data.Date_ferianatg2) : undefined
    }));
    console.log("Récuperation des données:", this.ferianatg2);

    // Preparing data for the charts
    const labels = this.ferianatg2
      .filter(data => !!data.Date_ferianatg2) // Filter objects where Date_Ferianatg2 is defined
      .map(data => this.formatDate(data.Date_ferianatg2!)); // Use ! to indicate that Date_Ferianatg2 cannot be undefined
    const degradationData = this.ferianatg2
      .filter(data => !!data.Date_ferianatg2) // Filter objects where Date_Ferianatg2 is defined
      .map(data => data.Tauxdegradation_ferianatg2);
    const gasConsumptionData = this.ferianatg2
      .filter(data => !!data.Date_ferianatg2) // Filter objects where Date_Ferianatg2 is defined
      .map(data => data.consogaz_ferianatg2);
    const defconsoData = this.ferianatg2
      .filter(data => !!data.Date_ferianatg2) // Filter objects where Date_Ferianatg2 is defined
      .map(data => data.defconso_ferianatg2);
    const rendementData = this.ferianatg2
      .filter(data => !!data.Date_ferianatg2) // Filter objects where Date_Ferianatg2 is defined
      .map(data => data.rendement_ferianatg2);
