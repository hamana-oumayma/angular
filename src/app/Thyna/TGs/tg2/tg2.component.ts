...
export class TG2Component implements OnInit {
  ...
  constructor(private thynatg2Service: Thynatg2Service, private http: HttpClient) { }
  ...
  getThynatg2(): void {
    this.thynatg2Service.getVal().subscribe(thynatg2 => {
      ...
      this.thynatg2 = thynatg2.map(data => ({
        ...data,
        Date_Thynatg2: data.Date_thynatg2 ? new Date(data.Date_thynatg2) : undefined
      }));
      console.log("Récuperation des données:", this.thynatg2);
  
      // Preparing data for the charts
      const labels = this.thynatg2
        .filter(data => !!data.Date_thynatg2) // Filter objects where Date_Thynatg2 is defined
        .map(data => this.formatDate(data.Date_thynatg2!)); // Use ! to indicate that Date_Thynatg2 cannot be undefined
      const degradationData = this.thynatg2
        .filter(data => !!data.Date_thynatg2) // Filter objects where Date_Thynatg2 is defined
        .map(data => data.Tauxdegradation_thynatg2);
      const gasConsumptionData = this.thynatg2
        .filter(data => !!data.Date_thynatg2) // Filter objects where Date_Thynatg2 is defined
        .map(data => data.consogaz_thynatg2);
      const defconsoData = this.thynatg2
        .filter(data => !!data.Date_thynatg2) // Filter objects where Date_Thynatg2 is defined
        .map(data => data.defconso_thynatg2);
      const rendementData = this.thynatg2
        .filter(data => !!data.Date_thynatg2) // Filter objects where Date_Thynatg2 is defined
        .map(data => data.rendement_thynatg2);
...
.subscribe((data: any) => {
  ...
  a.href = url;
  a.download = 'thynatg2_export.xls';
