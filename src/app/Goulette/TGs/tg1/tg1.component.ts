...
export class TG1Component implements OnInit {
  ...
  constructor(private goulettetg1Service: Goulettetg1Service, private http: HttpClient) { }
  ...
  getGoulettetg1(): void {
    this.goulettetg1Service.getVal().subscribe(goulettetg1 => {
      // Assigning data without filtering
      this.goulettetg1 = goulettetg1.map(data => ({
        ...data,
        Date_Goulettetg1: data.Date_goulettetg1 ? new Date(data.Date_goulettetg1) : undefined
      }));
      console.log("Récuperation des données:", this.goulettetg1);
  
      // Preparing data for the charts
      const labels = this.goulettetg1
        .filter(data => !!data.Date_goulettetg1) // Filter objects where Date_Goulettetg1 is defined
        .map(data => this.formatDate(data.Date_goulettetg1!)); // Use ! to indicate that Date_Goulettetg1 cannot be undefined
      const degradationData = this.goulettetg1
        .filter(data => !!data.Date_goulettetg1) // Filter objects where Date_Goulettetg1 is defined
        .map(data => data.Tauxdegradation_goulettetg1);
      const gasConsumptionData = this.goulettetg1
        .filter(data => !!data.Date_goulettetg1) // Filter objects where Date_Goulettetg1 is defined
        .map(data => data.consogaz_goulettetg1);
      const defconsoData = this.goulettetg1
        .filter(data => !!data.Date_goulettetg1) // Filter objects where Date_Goulettetg1 is defined
        .map(data => data.defconso_goulettetg1);
      const rendementData = this.goulettetg1
        .filter(data => !!data.Date_goulettetg1) // Filter objects where Date_Goulettetg1 is defined
        .map(data => data.rendement_goulettetg1);
...
.subscribe((data: any) => {
  ...
  a.href = url;
  a.download = 'goulettetg1_export.xls';
